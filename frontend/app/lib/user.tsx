export async function getAdmins(token: string) {
	try {
		...
		return {
			success: false,
			message: "Network error occurred while fetching admins",
		};
	}
}
...
export async function updateAdminRole(
	...
	try {
		const res = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/users/admins/update-role`,
			{
				method: "PATCH",
				headers: {
					Authorization: `Bearer ${token}`,
...
export async function getUserByUsername(username: string) {
	try {
		const res = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/auth/user/${username}`,
			{
				method: "GET",
				headers: {
					"Content-Type": "application/json",
