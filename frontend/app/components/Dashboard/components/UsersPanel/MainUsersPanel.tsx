...
const MainUsersPanel: React.FC = () => {
    ...
    useEffect(() => {
        const loadUsers = async () => {
            setLoading(true);
            const token = localStorage.getItem("anisync_token")?.replace(/^"|"$/g, '');
            if (!token) return; 
            
            const result = await fetchUsers(token);
            if (result.success) {
                const dataArray = Array.isArray(result.data) ? result.data : (result.data.users || []);
                const mappedUsers: User[] = dataArray.map((u: any) => ({
                    ...
                    joined: u.created_at ? new Date(u.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : "N/A"
                }));
