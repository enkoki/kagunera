	...
	return (
		<div className="flex justify-between gap-5">
			{stats.map((stat) => (
				<div
					key={stat.label}
					className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-[#312f40] bg-[#1b1922]"
				>
					<div className="shrink-0">{stat.icon}</div>
					<div>
						<div
							className={`text-2xl font-extrabold leading-tight ${stat.valueColor}`}
						>
							{stat.value}
						</div>
