	...
	if (cover) {
		return (
			<img
				src={cover}
				alt={title}
				className="w-[60px] h-[80px] rounded-md shrink-0 object-cover"
			/>
		);
	}
	return (
		<div className="w-[60px] h-[80px] rounded-md shrink-0 bg-[#1e1a2e] border border-[#2a2440] flex items-center justify-center overflow-hidden">
			<span className="text-[7px] text-gray-600 text-center px-1 leading-tight">
				{title.slice(0, 14)}
			</span>
		</div>
	);
	...
	<div>
		...
		<div className="flex flex-col">
			{items.map((item, i) => (
				...
				>
					<CoverPlaceholder title={item.title} cover={item.cover} />
